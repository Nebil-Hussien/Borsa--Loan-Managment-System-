...
class UserController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->middleware(['permission:user.users.Qrgenerator'])->only(['Qrgenerator']);
    }
    ...
    public function create()
    {
        ...
        $branches = Branch::all();
        $coop = Coop::all();
        return theme_view('user::user.create', compact('branches', 'cities', 'custom_fields', 'roles','coop'));
    }
    ...
    public function Qrgenerator($id)
    {
			...
			$user = User::find($id);
			if ($user) {
                if($user->QRpassword == NULL)
                {
				$qrLogin=bcrypt($user->id.$user->email);
		        $user->QRpassword= $qrLogin;
		        $user->update();
		        $result=1;
                }
        return theme_view('user::user.qr_generated', compact('user'));
        }else
        {
    ...
    public function edit($id)
    {
        ...
        $custom_fields = CustomField::where('category', 'add_user')->where('active', 1)->get();
        return theme_view('user::user.edit', compact('branches', 'user', 'selected_roles', 'custom_fields', 'roles', 'cities', 'coop'));
    }
