...
use Modules\Loan\Charts\LoanAmountInNumberRejectedPerGenderPieChart;
use Modules\Loan\Entities\Loan;
use Illuminate\Support\Facades\Auth;

class LoanAmountInNumberRejectedPerGender extends AbstractWidget{
    /**
     ...
     */
     public function run()
    {
        $chart = new LoanAmountInNumberRejectedPerGenderPieChart();
        $labels = [];
        $data = [];
        $colors=[];
    ...
    elseif(Auth::user()->hasRole('NGO_OperationOfficer')){
        ...
        $chart->title(trans_choice('core::general.loan_rejected_number_per_gender', 1));
        $chart->dataset("Number of Loan Rejected Per Gender",'pie',array_values($data))->color(array_values($colors));
        return theme_view('loan::widgets.loan_amount_in_number_rejected_per_gender', [
            'config' => $this->config,
            'chart'=>$chart
        ]);
    }
