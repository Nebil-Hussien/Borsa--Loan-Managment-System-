...
use Modules\Loan\Charts\LoanAmountInAmountApprovedPerGenderBarChart;
use Modules\Loan\Entities\Loan;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Auth;

class LoanAmountInAmountApprovedPerGender extends AbstractWidget{
    /**
     ...
     */
     public function run()
    {
        $chart = new LoanAmountInAmountApprovedPerGenderBarChart();
        $labels = [];
        $labels1 = [];
        $data = [];
        $data1 = [];
...
$data = Loan::join("clients", "clients.id", "loans.client_id")
    ...
    $chart->title(trans_choice('core::general.loan_disbursed_approved_per_gender', 1));
    ...
    $chart->dataset(trans_choice('core::general.male', 1), 'bar', array_values($expected1))->color('blue');
        ...
        return theme_view('loan::widgets.loan_amount_in_amount_approved_per_gender', [
            'config' => $this->config,
            'chart'=>$chart
        ]);
    }
