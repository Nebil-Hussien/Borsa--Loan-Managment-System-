...
use Modules\Loan\Charts\LoanAmountInNumberPerGenderBarChart;
use Modules\Loan\Entities\Loan;
use Illuminate\Support\Facades\Auth;

class LoanAmountInNumberPerGender extends AbstractWidget{
    /**
     ...
     */
     public function run()
    {
        $chart = new LoanAmountInNumberPerGenderBarChart();
        $labels = [];
        $data = [];
        $colors=[];
     ...
     elseif(Auth::user()->hasRole('NGO_OperationOfficer'))
    {
        ...
        $chart->title(trans_choice('core::general.loan_disbursed_number_per_gender', 1));
        $chart->dataset("Number of Loan Applied Per Gender",'pie',array_values($data))->color(array_values($colors));
        return theme_view('loan::widgets.loan_amount_in_number_per_gender', [
            'config' => $this->config,
            'chart'=>$chart
        ]);
    }
