...
use Modules\Loan\Charts\LoanAmountInNumberDisbursedPerGenderPieChart;
use Modules\Loan\Entities\Loan;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Auth;

class LoanAmountInNumberDisbursedPerGender extends AbstractWidget{
    /**
     ...
     */
     public function run()
    {
         $chart = new LoanAmountInNumberDisbursedPerGenderPieChart();
        $labels = [];
        $data = [];
        $colors=[];
         if(Auth::user()->hasRole('NGO_HeadOffice')){
          ...
          ->selectRaw("count(loans.client_id) count, gender")
          ->groupBy('gender')
          ->get() as $key) {                
    ...
    }elseif(Auth::user()->hasRole('NGO_OperationOfficer'))
    {
        ...
        $chart->title(trans_choice('core::general.loan_actual_disbursed_number_per_gender', 1));
        $chart->dataset("Number of Loan Disbursed Per Gender",'pie',array_values($data))->color(array_values($colors));
        return theme_view('loan::widgets.loan_amount_in_number_disbursed_per_gender', [
            'config' => $this->config,
            'chart'=>$chart
        ]);
    }
