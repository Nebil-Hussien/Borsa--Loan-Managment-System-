...
use Modules\Loan\Charts\LoanAmountInNumberPerNGOPerGenderBarChart;
use Modules\Loan\Entities\Loan;

class LoanAmountInNumberPerNGOPerGender extends AbstractWidget{
    /**
     ...
     */
     public function run()
    {
        $chart = new LoanAmountInNumberPerNGOPerGenderBarChart();
        $labels = [];
        $data = [];
        $colors=[];
        $data1=[];
    ...
    ->get() as $line)
    {
        ...
        $chart->title(trans_choice('core::general.loan_application_number_per_ngo_per_gender', 1));
        ...
        return theme_view('loan::widgets.loan_amount_in_number_per_ngo_per_gender', [
            'config' => $this->config,
            'chart'=>$chart
        ]);
    }
