...
use Modules\Loan\Charts\LoanAmountInNumberWithdrawnPerGenderPieChart;
use Modules\Loan\Entities\Loan;
use Illuminate\Support\Facades\Auth;

class LoanAmountInNumberWithdrawnPerGender extends AbstractWidget{
    /**
     ...
     */
     public function run()
    {
        $chart = new LoanAmountInNumberWithdrawnPerGenderPieChart();
        $labels = [];
        $data = [];
        $colors=[];
    ...
    elseif(Auth::user()->hasRole('NGO_OperationOfficer'))
    {
        ...
        $chart->title(trans_choice('core::general.loan_withdrawn_number_per_gender', 1));
        $chart->dataset("Number of Loan Withdrawn Per Gender",'pie',array_values($data))->color(array_values($colors));
        return theme_view('loan::widgets.loan_amount_in_number_withdrawn_per_gender', [
            'config' => $this->config,
            'chart'=>$chart
        ]);
    }
