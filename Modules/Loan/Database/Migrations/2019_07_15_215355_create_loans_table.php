...
class CreateLoansTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('loans', function (Blueprint $table) {
            ...
            $table->text('bank_saving_account',255)-> nullable();
            $table->boolean('martial_status');
            $table->text('tin_number',255)->nullable();
            $table->text('tin_number2',255)->nullable();
            $table->bigInteger('bussiness_sector_id',20)->unsigned()->nullable();
            $table->text('owners_of_bussiness',255)->nullable();
            $table->text('reason_bussiness',255)->nullable();
            $table->boolean('physical_bussiness');
            $table->bigInteger('estimated_market_size')->default(0);
            $table->text('market_potential')->nullable();
            $table->text('sales_approach')->nullable();
            $table->bigInteger('direct_employee')->default(0);
            $table->bigInteger('indirect_employee')->default(0);
            $table->boolean('supplier_amount');
            $table->boolean('loan_before');
            $table->bigInteger('number_rounds_loan')->default(0);
            $table->boolean('repayment_history_status');
            $table->text('reason_histroy_loan')->nullable();
            $table->float('current_asset_amount')->default(0);
            $table->float('current_liability_amount')->default(0);
            $table->float('long_term_liabilit_amount')->default(0);
            $table->float('annual_income_bussiness')->default(0);
            $table->float('monthly_income_bussiness')->default(0);
            $table->float('daily_income_bussiness')->default(0);
            $table->float('other_source_of_income')->default(0);
            $table->float('annual_expense')->default(0);
            $table->float('monthly_expense')->default(0);
            $table->float('daily_expesnse')->default(0);
            $table->boolean('book_of_records');
            $table->bigInteger('knowledge_id')->default(NULL);
            $table->text('expected_risk_of_bussiness')->default(NULL);
            $table->text('other_requirements')->default(NULL);
            $table->boolean('edir_status');
            $table->boolean('edir_payment_status')->default(null);
            $table->boolean('equib_status');
            $table->boolean('equib_payment_status')->default(NULL);
            $table->boolean('community_role_status')->default(NULL);
            $table->boolean('utillites_payment_status')->default(NULL);
            $table->boolean('fines_penalities_status')->default(NULL);
            $table->timestamps();
            ...
            $table->index('bussiness_sector_id');
