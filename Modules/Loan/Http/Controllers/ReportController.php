...
class ReportController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware(['permission:loan.loans.reports.mfi_repayment'])->only(['mfi_repayment']);
        $this->middleware(['permission:loan.loans.reports.general_mfi_repayment'])->only(['general_mfi_repayment']);
    ...
    {
        $user_id = Auth::user();
        if ($user_id->branch_id && $user_id->city_id) {
            $users = User::whereHas('roles', function ($query) {
                return $query->where('name', '!=', 'client');
            })->where('branch_id', $user_id->branch_id)->where('city_id', $user_id->city_id)
                ->get();
        } elseif ($user_id->branch_id) {
            $users = User::whereHas('roles', function ($query) {
                return $query->where('name', '!=', 'client');
            })->where('branch_id', $user_id->branch_id)
                ->get();
        } else {
            $users = User::whereHas('roles', function ($query) {
                return $query->where('name', '!=', 'client');
        ...
        $branch_id = determin_the_role_forLoanView_branch($user_id, $request_forRestBranch);
        $city_id =  determin_the_role_forLoanView_city($user_id, $request_forRestCity);
        $loan_officer_id = determin_the_role_forLoanView_loan_officer($user_id, $request_forRestLoan_officer);
    ...
    public function DM_collection_sheet(Request $request)
    {
        $user_id = Auth::user();
        $branch_id = determin_the_role_forLoanView_branch($user_id);
        $city_id =  determin_the_role_forLoanView_city($user_id);
        $start_date = $request->start_date;
        $end_date = $request->end_date;
        $loan_product_id = $request->loan_product_id;
        $loan_products = LoanProduct::all();
        ...
        if (!empty($start_date)) {
            $data = DB::table("loan_repayment_schedules")
                ...
                ->where('loans.status', 'active')
                ->selectRaw("concat(clients.first_name,' ',clients.middle_name) client,branches.name branch,clients.mobile,loans.client_id,loan_products.name loan_product,loan_repayment_schedules.loan_id,loans.expected_maturity_date,loan_repayment_schedules.total_due,(loan_repayment_schedules.principal+loan_repayment_schedules.interest+loan_repayment_schedules.fees+loan_repayment_schedules.penalties-loan_repayment_schedules.principal_written_off_derived-loan_repayment_schedules.interest_written_off_derived-loan_repayment_schedules.fees_written_off_derived-loan_repayment_schedules.penalties_written_off_derived-loan_repayment_schedules.interest_waived_derived-loan_repayment_schedules.fees_waived_derived-loan_repayment_schedules.penalties_waived_derived) expected_amount,loan_repayment_schedules.due_date")
                ->get();
            //check if we should download
            if ($request->download) {
                if ($request->type == 'pdf') {
     ...
     * @return Response
     */
    public function mfi_repayment(Request $request)
    {
        $user_id = Auth::user();        
        $cities = Cities::all();
        $branches = Branch::all();
        $request_forRestBranch = $request->branch_id;
        $request_forRestCity = $request->city_id;
        $request_forRestLoan_officer = $request->repayment_officer_id;
        $request_forRestCoop = $request->coop_id;
        $branch_id = determin_the_role_forLoanView_branch($user_id, $request_forRestBranch);
        $city_id =  determin_the_role_forLoanView_city($user_id, $request_forRestCity);
        $loan_officer_id = determin_the_role_forLoanView_loan_officer($user_id, $request_forRestLoan_officer);
        $coop_id = determin_the_role_forLoanView_coop_office($user_id, $request_forRestCoop);
        $start_date = $request->start_date;
        $end_date = $request->end_date;
        if ($user_id->coop_id && $user_id->city_id) {
            $users = User::whereHas('roles', function ($query) {
                return $query->where('name', '!=', 'client');
            })->where('coop_id', $user_id->coop_id)->where('city_id', $user_id->city_id)
                ->get();
        } elseif ($user_id->coop_id) {
            $users = User::whereHas('roles', function ($query) {
                return $query->where('name', '!=', 'client');
            })->where('coop_id', $user_id->coop_id)
                ->get();
        } else {
            $users = User::whereHas('roles', function ($query) {
                return $query->where('name', '!=', 'client');
        ...
        if (!empty($start_date)) {
            $data = DB::table("loan_transactions")
                ...
                ->when($coop_id, function ($query) use ($coop_id) {
                    $query->where('loan_transactions.coop_id', $coop_id);
                })
     ...
     public function general_mfi_repayment(Request $request)
    {
        $user_id = Auth::user();
        if ($user_id->coop_id && $user_id->city_id) {
            $users = User::whereHas('roles', function ($query) {
        ...
        if (!empty($start_date)) {
            $data = DB::table("loan_transactions")
                ->join("loans", "loan_transactions.loan_id", "loans.id")
                ->join("coop", "loan_transactions.coop_id", "coop.id")
                ->join("branches", "loans.branch_id", "branches.id")
                ->join("clients", "loans.client_id", "clients.id")
                ->leftJoin("payment_details", "loan_transactions.payment_detail_id", "payment_details.id")
                ->leftJoin("users", "loan_transactions.created_by_id", "users.id")
                ...
                ->where('loan_transaction_type_id', 2)
                ->selectRaw("concat(clients.first_name,' ',clients.middle_name) client,concat(coop.name,' ') coop,branches.name branch,loans.client_id,loan_transactions.id,loan_transactions.loan_id,loan_transactions.principal_repaid_derived,loan_transactions.interest_repaid_derived,loan_transactions.fees_repaid_derived,loan_transactions.penalties_repaid_derived,loan_transactions.submitted_on,payment_types.name payment_type")
                ->get();
            //check if we should download
            if ($request->download) {
                if ($request->type == 'pdf') {
                    $pdf = PDF::loadView(theme_view_file('loan::report.general_mfi_repayment_pdf'), compact(
                        'start_date',
                        'end_date',
                        'branch_id',
                        'data',
                ...
                $view = theme_view(
                    'loan::report.general_mfi_repayment_pdf',
                    compact(
                        'start_date',
                        'end_date',
                        'branch_id',
        ...
        return theme_view(
            'loan::report.general_mfi_repayment',
            compact(
                'start_date',
                'end_date',
                'branch_id',
    ...
    public function loan_status_report(Request $request)
    {
        ...
        if (!empty($start_date)) {
            $data = DB::table("loans")
                ...
                ->selectRaw("clients.id,concat(clients.first_name,' ',clients.middle_name) client,
    ...
    public function loan_application_report(Request $request)
    {
        ...
        if (!empty($start_date)) {
            $data = DB::table("loan_applications")
                ...
                ->selectRaw("loan_applications.id,  clients.id client_ID, concat(clients.first_name,' ',clients.middle_name) client,
                loan_products.name loan_products, branches.name branches, funds.name funds,
    ...
    public function client_reports(Request $request)
    {
        ...
        if (!empty($branch_id)) {
            $data = DB::table("clients")
                ...
                ->selectRaw("clients.id,concat(clients.first_name,' ',clients.middle_name,' ',clients.middle_name) client,clients.gender, clients.age, clients.mobile, clients.email, clients.status, clients.created_at, branches.name branches")
                ->get();
            //check if we should download
            if ($request->download) {
                if ($request->type == 'pdf') {
