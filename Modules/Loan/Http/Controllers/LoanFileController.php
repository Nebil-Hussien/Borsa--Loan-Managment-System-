...
class LoanFileController extends Controller
{
    public function __construct()
    {
		...
		$this->middleware(['permission:loan.loans.files.download'])->only(['download']);
    ...
    public function store(Request $request, $id)
    {
       $request->validate([
        ...
        if ($request->hasFile('file')) {
            ...
            $request->file->move('assets/uploads/loan', $file_name);
    ...
    {
    	return response()->download(public_path('assets/uploads/loan/'.$link));
    }
    ...
    public function destroy($id)
    {
        $loan_file = LoanFile::find($id);
        if ($loan_file->link) {
            Storage::delete('public/uploads/loan/' . $loan_file->link);
        }
        $loan_file->delete();
        activity()->on($loan_file)
            ->withProperties(['id' => $loan_file->id])
