            ...
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>{{ trans_choice('loan::general.loan', 1) }}
                        {{ trans_choice('loan::general.repayment', 1) }}</h1>
                </div>
                <div class="col-sm-6">
                ...
                <div class="card-tools hidden-print">
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-xs dropdown-menu-right">
                            <a href="{{ url('report/loan/repayment?download=1&type=csv&start_date=' . $start_date . '&end_date=' . $end_date . '&coop_id=' . $coop_id . '&branch_id=' . $branch_id . '&city_id=' . $city_id) }}"
                                class="dropdown-item">{{ trans_choice('core::general.download', 1) }}
                                {{ trans_choice('core::general.csv_format', 1) }}</a>
                            <a href="{{ url('report/loan/repayment?download=1&type=excel&start_date=' . $start_date . '&end_date=' . $end_date . '&coop_id=' . $coop_id . '&branch_id=' . $branch_id . '&city_id=' . $city_id) }}"
                                class="dropdown-item">{{ trans_choice('core::general.download', 1) }}
                                {{ trans_choice('core::general.excel_format', 1) }}</a>
                            <a href="{{ url('report/loan/repayment?download=1&type=excel_2007&start_date=' . $start_date . '&end_date=' . $end_date . '&coop_id=' . $coop_id . '&branch_id=' . $branch_id . '&city_id=' . $city_id) }}"
                                class="dropdown-item">{{ trans_choice('core::general.download', 1) }}
                                {{ trans_choice('core::general.excel_2007_format', 1) }}</a>
                            <a href="{{ url('report/loan/repayment?download=1&type=pdf&start_date=' . $start_date . '&end_date=' . $end_date . '&coop_id=' . $coop_id . '&branch_id=' . $branch_id . '&city_id=' . $city_id) }}"
                                class="dropdown-item">{{ trans_choice('core::general.download', 1) }}
                ...
                <form method="get" action="{{ Request::url() }}" class="">
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="control-label"
                                    for="coop_id">{{ trans_choice('core::general.coop', 1) }}</label>
                                <select class="form-control select2" name="coop_id" id="coop_id" required>
                                    <option value="" disabled selected>
                                        {{ trans_choice('core::general.select', 1) }}
                                    </option>
                                    @foreach ($coop as $key)
                                        <option value="{{ $key->id }}"
                                            @if ($coop_id == $key->id) selected @endif>{{ $key->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <table class="table table-bordered table-condensed table-hover">
                        <thead>
                            <tr>
                                ...
                                <th colspan="2">
                                    @if (!empty($data->first()) && !empty($coop_id))
                                        {{ trans_choice('core::general.branch', 1) }}:

                                        {{ $data->first()->coop }}
                                    @endif
                                </th>
                                <th colspan="2">{{ trans_choice('core::general.start_date', 1) }}:
                                    {{ $start_date }}</th>
                            ...
                            <tr style="background-color: #D1F9FF">
                                ...
                                <th>{{ trans_choice('core::general.coop', 1) }}</th>
                                ...
                                <th>{{ trans_choice('core::general.payment', 1) }}
