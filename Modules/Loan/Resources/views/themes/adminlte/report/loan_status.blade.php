...
@section('title')
    {{ trans_choice('loan::general.loan_status_report', 1) }}
@endsection
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>{{ trans_choice('loan::general.loan_status_report', 1) }}
                    </h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">{{ trans_choice('loan::general.loan_status_report', 1) }}
                        </li>
                    </ol>
                </div>
            </div>
                ...
                <div class="card-tools hidden-print">
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-xs dropdown-menu-right">
                            <a href="{{ url('report/loan/loan_status?download=1&type=csv&&start_date=' . $start_date . '&end_date=' . $end_date . '&branch_id=' . $branch_id . /*'&loan_officer_id='.$loan_officer_id. */ '&loan_product_id=' . $loan_product_id . '&city_id=' . $city_id) }}"
                                class="dropdown-item">{{ trans_choice('core::general.download', 1) }}
                                {{ trans_choice('core::general.csv_format', 1) }}</a>
                            <a href="{{ url('report/loan/loan_status?download=1&type=excel&&start_date=' . $start_date . '&end_date=' . $end_date . '&branch_id=' . $branch_id . /* '&loan_officer_id='.$loan_officer_id. */ '&loan_product_id=' . $loan_product_id . '&city_id=' . $city_id) }}"
                                class="dropdown-item">{{ trans_choice('core::general.download', 1) }}
                                {{ trans_choice('core::general.excel_format', 1) }}</a>
                            <a href="{{ url('report/loan/loan_status?download=1&type=excel_2007&&start_date=' . $start_date . '&end_date=' . $end_date . '&branch_id=' . $branch_id . /* '&loan_officer_id='.$loan_officer_id.*/ '&loan_product_id=' . $loan_product_id . '&city_id=' . $city_id) }}"
                                class="dropdown-item">{{ trans_choice('core::general.download', 1) }}
                                {{ trans_choice('core::general.excel_2007_format', 1) }}</a>
                            <a href="{{ url('report/loan/loan_status?download=1&type=pdf&&start_date=' . $start_date . '&end_date=' . $end_date . '&branch_id=' . $branch_id . /* '&loan_officer_id='.$loan_officer_id .*/ '&loan_product_id=' . $loan_product_id . '&city_id=' . $city_id) }}"
                                class="dropdown-item">{{ trans_choice('core::general.download', 1) }}
                    ...
                    <table class="table table-bordered table-condensed table-hover">
                        <thead>
                            ...
                            <tr style="background-color: #D1F9FF">
                                ...
                                <th>{{ trans_choice('loan::general.disbursed_notes', 1) }}</th>
                                <th>{{ trans_choice('loan::general.rejected_on_date', 1) }}</th>
                                <th>{{ trans_choice('loan::general.rejected_Notes', 1) }}</th>
                        ...
                        <tbody>
                            ...
                            @foreach ($data as $key)
                                <tr>
                                    ...
                                    <td>{{ $key->disbursed_notes }}</td>
                                    <td>{{ $key->rejected_on_date }}</td>
                                    <td>{{ $key->rejected_notes }}</td>
