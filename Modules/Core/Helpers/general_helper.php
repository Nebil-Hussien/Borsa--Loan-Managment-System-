...
if (!function_exists('determine_the_city')) {
    function determine_the_city($cities)
    {
        //$data = ClientCity::Where('shortName', $cities)->select("cities.name");
        $data = DB::table('cities')->where('shortName', $cities)->value('name');
        //$string = implode(" ", $data);
        return $data;
    }
}
if (!function_exists('determin_the_role_forLoanView_branch')) {
    function determin_the_role_forLoanView_branch($authUser, $request_forRestBranch)
    {
		$authRole =  $authUser->roles->pluck('name');
        $auth = $authRole[0];     
        if($auth=="Head_Office"|| $auth == "admin"|| $auth == "Partners"|| $auth == "COOP_OperationOfficerRepayment"|| $auth == "COOP_RegionalManagmentRepayment"|| $auth == "COOP_HeadOfficeRepayment" || $auth == "Regional_Office")
		{
			return $request_forRestBranch;
        }elseif($auth == "NGO_HeadOffice" || $auth =="NGO_RegionalManagment" || $auth == "NGO_OperationOfficer" )
        {
...
if (!function_exists('determin_the_role_forLoanView_city')) {
    function determin_the_role_forLoanView_city($authUser, $request_forRestCity)
    {
        $authRole =  $authUser->roles->pluck('name');
        $auth = $authRole[0];
		 if ($auth == "NGO_HeadOffice" || $auth == "COOP_OperationOfficerRepayment" || $auth == "admin" || $auth == "Partners" || $auth == "Head_Office") {
            return $request_forRestCity;
        }
        elseif ($auth == "Regional_Office" || $auth == "NGO_OperationOfficer" || $auth =="NGO_RegionalManagment") {
            return $authUser->city_id;
        }
       
    }
}
if (!function_exists('determin_the_role_forLoanView_loan_officer')) {
    function determin_the_role_forLoanView_loan_officer($authUser, $request_forRestLoan_officer)
    {
        $authRole =  $authUser->roles->pluck('name');
        $auth = $authRole[0];
        if ($auth =="NGO_RegionalManagment" || $auth == "COOP_RegionalManagmentRepayment" || $auth == "Regional_Office" || $auth == "NGO_HeadOffice" || $auth == "COOP_HeadOfficeRepayment" || $auth == "Head_Office" || $auth == "admin"|| $auth == "Partners") {
              return $request_forRestLoan_officer;
          
        } elseif ($auth == "NGO_OperationOfficer" || $auth == "COOP_OperationOfficerRepayment") 
        {
            return $authUser->id;          
...
if (!function_exists('determin_the_role_forLoanView_coop_office')) {
    function determin_the_role_forLoanView_coop_office($authUser, $request_forRestCoop)
    {
        $authRole =  $authUser->roles->pluck('name');
        $auth = $authRole[0];
        if ($auth == "COOP_RegionalManagmentRepayment" || $auth == "COOP_HeadOfficeRepayment" || $auth == "COOP_OperationOfficerRepayment") {
             return $authUser->coop_id;
          
        } elseif ($auth == "admin") 
        {
             return $request_forRestCoop;
        }
    }
}
if (!function_exists('determin_the_role_forClientView')) {
    function determin_the_role_forClientView($authUser)
    {
        $authRole =  $authUser->roles->pluck('name');
        $auth = $authRole[0];
        if ($auth == "admin" || $auth == "Head_Office") {
            return "";
        }
    }
}