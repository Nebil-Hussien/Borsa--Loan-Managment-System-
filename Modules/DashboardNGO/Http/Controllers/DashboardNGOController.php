...
namespace Modules\DashboardNGO\Http\Controllers;
...
use Modules\DashboardNGO\Entities\Widget;
use Modules\Loan\Entities\Loan;
use Nwidart\Modules\Facades\Module;

class DashboardNGOController extends Controller
{
     public function __construct()
    {
        $this->middleware(['auth', '2fa']);
        $this->middleware(['permission:dashboardNgo.edit'])->only(['create', 'store_widget', 'edit', 'update', 'update_widget_positions']);
        $this->middleware(['permission:dashboardNgo.destroy'])->only(['destroy']);
    ...
    public function index()
    {
        ...
        return theme_view('dashboardngo::dashboard',compact('available_widgets', 'user_widgets'));
    }
    ...
    {
        return theme_view('dashboardngo::create');
    }
    ...
    {
        return theme_view('dashboardngo::show');
    }
    ...
    {
        return theme_view('dashboardngo::edit');
    }
    ...
    public function remove_widget(Request $request){
        ...
        if (!empty($user_widgets)) {
            $widgets = json_decode($user_widgets->widgets, true);
            unset($widgets[$request->widget_id]);
            $user_widgets->widgets = json_encode($widgets);
            $user_widgets->save();

        }
        Flash::warning(trans_choice("core::general.successfully_deleted", 1));
         return redirect()->back();
