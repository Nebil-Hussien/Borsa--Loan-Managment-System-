...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>{{ trans_choice('loan::general.loan', 1) }}
                        {{ trans_choice('loan::general.disbursement', 1) }}</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">{{ trans_choice('loan::general.loan', 1) }}
                            {{ trans_choice('loan::general.disbursement', 1) }}</li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
                ...
                <div class="card-tools hidden-print">
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-xs dropdown-menu-right">
                            <a href="{{ url('report/loan/disbursement?download=1&type=csv&&start_date=' . $start_date . '&end_date=' . $end_date . '&branch_id=' . $branch_id . /*'&loan_officer_id='.$loan_officer_id. */ '&loan_product_id=' . $loan_product_id) }}"
                                class="dropdown-item">{{ trans_choice('core::general.download', 1) }}
                                {{ trans_choice('core::general.csv_format', 1) }}</a>
                            <a href="{{ url('report/loan/disbursement?download=1&type=excel&&start_date=' . $start_date . '&end_date=' . $end_date . '&branch_id=' . $branch_id . /* '&loan_officer_id='.$loan_officer_id. */ '&loan_product_id=' . $loan_product_id) }}"
                                class="dropdown-item">{{ trans_choice('core::general.download', 1) }}
                                {{ trans_choice('core::general.excel_format', 1) }}</a>
                            <a href="{{ url('report/loan/disbursement?download=1&type=excel_2007&&start_date=' . $start_date . '&end_date=' . $end_date . '&branch_id=' . $branch_id . /* '&loan_officer_id='.$loan_officer_id.*/ '&loan_product_id=' . $loan_product_id) }}"
                                class="dropdown-item">{{ trans_choice('core::general.download', 1) }}
                                {{ trans_choice('core::general.excel_2007_format', 1) }}</a>
                            <a href="{{ url('report/loan/disbursement?download=1&type=pdf&&start_date=' . $start_date . '&end_date=' . $end_date . '&branch_id=' . $branch_id . /* '&loan_officer_id='.$loan_officer_id .*/ '&loan_product_id=' . $loan_product_id) }}"
                                class="dropdown-item">{{ trans_choice('core::general.download', 1) }}
                    ...
                    <table class="table table-bordered table-striped table-condensed table-hover">
                        <thead>
                            <tr>
                                ...
                                <th colspan="4">{{ trans_choice('core::general.start_date', 1) }}: {{ $start_date }}
                                </th>
                                <th colspan="4">{{ trans_choice('core::general.end_date', 1) }}: {{ $end_date }}
                                </th>
                            </tr>
                            <tr>
                                <th colspan="8"></th>
                            ...
                            <tr style="background-color: #D1F9FF">
                                ...
                                <th>{{ trans_choice('loan::general.disbursed', 1) }}
                                ...
                                <th>{{ trans_choice('loan::general.arrears', 1) }}
                                    {{ trans_choice('loan::general.amount', 1) }}</th>
                                <th>{{ trans_choice('loan::general.day', 2) }}
                        ...
                        <tbody>
                            ...
                            @foreach ($data as $key)
                                ...
                                <tr>
                                    ...
                                    <td>{{ number_format($key->repayment_schedules->sum('principal') + $key->repayment_schedules->sum('interest') + $key->repayment_schedules->sum('fees') + $key->repayment_schedules->sum('penalties'), 2) }}
                                    </td>
                                    ...
                                    <td>{{ number_format($principal_outstanding + $interest_outstanding + $fees_outstanding + $penalties_outstanding, 2) }}
                                    </td>
                                    <td>{{ $key->fund }}</td>
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <td>{{ number_format($total_principal + $total_interest + $total_fees + $total_penalties, 2) }}
                                </td>
                                ...
                                <td>{{ number_format($total_principal_outstanding + $total_interest_outstanding + $total_fees_outstanding + $total_penalties_outstanding, 2) }}
                                </td>
                                <td colspan="3"></td>
                                <td>{{ number_format($total_principal_overdue + $total_interest_overdue + $total_fees_overdue + $total_penalties_overdue, 2) }}
                                </td>
                                <td></td>
                            </tr>
                        </tfoot>
