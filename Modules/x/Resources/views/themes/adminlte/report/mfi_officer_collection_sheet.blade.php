                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">
                            {{ trans_choice('loan::general.mfi_officer_collection_sheet', 1) }}</li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
    ...
    <section class="content" id="app">
        <div class="card">
            <div class="card-header with-border">
                <h3 class="card-title">
                    {{ trans_choice('loan::general.mfi_officer_collection_sheet', 1) }}
                    @if (!empty($start_date))
                ...
                <div class="card-tools hidden-print">
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-xs dropdown-menu-right">
                            <a href="{{ url('report/loan/mfi_officer_collection_sheet?download=1&type=csv&start_date=' . $start_date . '&end_date=' . $end_date . '&branch_id=' . $branch_id . /* '&loan_officer_id=' . $loan_officer_id .  */ '&loan_product_id=' . $loan_product_id) }}"
                                class="dropdown-item">{{ trans_choice('core::general.download', 1) }}
                                {{ trans_choice('core::general.csv_format', 1) }}</a>
                            <a href="{{ url('report/loan/mfi_officer_collection_sheet?download=1&type=excel&start_date=' . $start_date . '&end_date=' . $end_date . '&branch_id=' . $branch_id . /* '&loan_officer_id=' . $loan_officer_id . */ '&loan_product_id=' . $loan_product_id) }}"
                                class="dropdown-item">{{ trans_choice('core::general.download', 1) }}
                                {{ trans_choice('core::general.excel_format', 1) }}</a>
                            <a href="{{ url('report/loan/mfi_officer_collection_sheet?download=1&type=excel_2007&start_date=' . $start_date . '&end_date=' . $end_date . '&branch_id=' . $branch_id . /*  '&loan_officer_id=' . $loan_officer_id . */ '&loan_product_id=' . $loan_product_id) }}"
                                class="dropdown-item">{{ trans_choice('core::general.download', 1) }}
                                {{ trans_choice('core::general.excel_2007_format', 1) }}</a>
                            <a href="{{ url('report/loan/mfi_officer_collection_sheet?download=1&type=pdf&start_date=' . $start_date . '&end_date=' . $end_date . '&branch_id=' . $branch_id . /* '&loan_officer_id=' . $loan_officer_id . */ '&loan_product_id=' . $loan_product_id) }}"
                                class="dropdown-item">{{ trans_choice('core::general.download', 1) }}
                ...
                <form method="get" action="{{ Request::url() }}" class="">
                    <div class="row">
                        ...
                        {{-- <div class="col-md-4">
                            <div class="form-group">
                                <label class="control-label"
                                    for="loan_officer_id">{{ trans_choice('loan::general.loan', 1) }}
                    ...
                    <table class="table table-bordered table-condensed table-hover">
                        <thead>
                            <tr>
                                ...
                                <th colspan="2">{{ trans_choice('core::general.start_date', 1) }}: {{ $start_date }}
                                </th>
                                <th colspan="2">{{ trans_choice('core::general.end_date', 1) }}: {{ $end_date }}
                                </th>
                            </tr>
                            <tr style="background-color: #D1F9FF">
                                ...
                                <th>{{ trans_choice('loan::general.repayment', 1) }}
                                    {{ trans_choice('core::general.date', 1) }}</th>
                                <th>{{ trans_choice('loan::general.expected', 1) }}
                                    {{ trans_choice('loan::general.amount', 1) }}</th>
                                <th>{{ trans_choice('loan::general.total', 1) }}
